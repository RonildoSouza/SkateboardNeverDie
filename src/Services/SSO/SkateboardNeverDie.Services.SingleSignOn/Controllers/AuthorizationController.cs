...
using SkateboardNeverDie.Core.Collection.Extensions;
using SkateboardNeverDie.Core.MVC.Attributes;
using SkateboardNeverDie.Services.SingleSignOn.Infrastructure;
using SkateboardNeverDie.Services.SingleSignOn.ViewModels.Authorization;
using System;
...
namespace SkateboardNeverDie.Services.SingleSignOn.Controllers
{
    ...
    public class AuthorizationController : Controller
    {
        ...
        public async Task<IActionResult> Exchange()
        {
            ...
            switch (request.GrantType)
            {
                case OpenIddictConstants.GrantTypes.ClientCredentials:
                    ...
                    //identity.AddClaim(OpenIddictConstants.Claims.Role, "admin", OpenIddictConstants.Destinations.AccessToken);
                    ...
                    if (!scopes.Any())
                    {
                        scopes = await _applicationManager.GetPermissionsAsync(application);
                        scopes = scopes.Where(_ => _.StartsWith(OpenIddictConstants.Permissions.Prefixes.Scope))
                            .Select(_ => _.Replace(OpenIddictConstants.Permissions.Prefixes.Scope, string.Empty))
                            .ToImmutableArray();
                    }
        ...
        public async Task<IActionResult> Authorize()
        {
            ...
            // If prompt=login was specified by the client application, immediately return the user agent to the login page.
        ...
        private static IEnumerable<string> GetDestinations(Claim claim, ClaimsPrincipal claimsPrincipal)
        {
            ...
            switch (claim.Type)
            {
                ...
                case OpenIddictConstants.Claims.Role:
                    ...
                    if (claimsPrincipal.HasScope(OpenIddictConstants.Scopes.Roles))
                        yield return OpenIddictConstants.Destinations.IdentityToken;
